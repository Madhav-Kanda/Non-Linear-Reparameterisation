...
def base(
    ...
    def fast_update(
        ...
        # new_position, _ = jax.flatten_util.ravel_pytree(position)
        num_size = warmup_state.parameters.shape[1]
        print(num_size,1)
        ...
        return WindowAdaptationState(
            ...
            jnp.concatenate((warmup_state.parameters, jnp.zeros((1, num_size))))
        )
    ...
    def slow_update(
        ...
        # jax.debug.print("New_imm_state:{new_imm_state}",new_imm_state = new_imm_state)

        return WindowAdaptationState(
            new_ss_state, new_imm_state, new_step_size, warmup_state.inverse_mass_matrix, jnp.concatenate((warmup_state.parameters, jnp.expand_dims(new_position, axis=0)), axis=0))
    ...
    def slow_final(warmup_state: WindowAdaptationState) -> WindowAdaptationState:
        ...
        new_step_size = jnp.exp(new_ss_state.log_step_size)
        # new_position, _ = jax.flatten_util.ravel_pytree(position)
        jax.debug.print("warmup parameters:{warmup_state}", warmup_state = warmup_state.parameters)
        num_size = warmup_state.parameters.shape[1]
        print(num_size,2)
    ...
    def update(
        ...
        # print(stage, is_middle_window_end)

        ## How does jax.lax.switch work? & jax.lax.cond?

        warmup_state = jax.lax.switch(
            stage,
            (fast_update, slow_update),  ## fast_update --> 0, slow_update --> 1 (Index based)
            position,
            acceptance_rate,
            adaptation_state,
        )  
        # jax.debug.print("Old:{warmup_state}",warmup_state = warmup_state)
        ## slow_update --> fast_update + updation of mass matrix
        
        print("before last step window changes")
        ...
        # jax.debug.print("New:{warmup_state}",warmup_state = warmup_state)
        # print(warmup_state)
        return warmup_state
...
def window_adaptation(
    ...
    **extra_parameters
    ...
    def one_step(carry, xs):
        ...
        ## adaptation_stage --> schedule
        ## adaptation_state --> adapt_init_state
    ...
    def run(rng_key: PRNGKey, position: ArrayLikeTree, num_steps: int = 1000):
        ...
        last_state, (states,info) = jax.lax.scan(
            one_step_,
        ...
        # print(states)
        print("Hello")
        jax.debug.print("States:{states}", states = states)
        return (
            AdaptationResults(
                last_chain_state,
                parameters,
